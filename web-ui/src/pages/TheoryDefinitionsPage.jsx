...
import { DEFAULT_DEFINITIONS } from "../constants/theoryData";
...
export default function TheoryDefinitionsPage({ onNavigate }) {
    ...
    if (stored) {
      try {
        ...
        return parsed.map((item, idx) => ({
          ...item,
          image: item.image || (DEFAULT_DEFINITIONS[idx] ? DEFAULT_DEFINITIONS[idx].image : defGeometricImg)
        }));
      } catch (e) {
        console.error(e);
      }
  ...
  const handleNavClick = (id) => {
    if (id === "concept") {
      ...
      document.getElementById("definitions-grid")?.scrollIntoView({ behavior: "smooth" });
    ...
    <div style={{ minHeight: "100vh", background: "#f8f8f8", fontFamily: "'Plus Jakarta Sans', 'Inter', sans-serif" }}>
      ...
      >
        ...
        <div className="hidden lg:block absolute inset-0 w-full h-full z-10">
          ...
          >
            <defs>
              <linearGradient id="sidebarGradD" x1="0" y1="0" x2="1" y2="1">
                <stop offset="0%" stopColor="#120c2b" stopOpacity="0.96" />
                <stop offset="100%" stopColor="#2e1a8a" stopOpacity="0.9" />
              </linearGradient>
            </defs>
            <polygon points="0,0 26,0 42,100 0,100" fill="url(#sidebarGradD)" />
            <line x1="26" y1="0" x2="42" y2="100" stroke="#592eff" strokeWidth="0.3" opacity="0.8" />
          </svg>

          {NAV_ITEMS.map((item) => {
            const isSelected = item.id === "definition";
            const xPercent = 26 + 0.16 * item.y;
            return (
              <React.Fragment key={item.id}>
                <div
                  className={`absolute rounded-full z-20 transition-all duration-300 ${
                    isSelected
                ...
                />
                <button
                  className={`absolute border-none bg-transparent font-sans text-right transition-all duration-300 font-extrabold uppercase tracking-widest cursor-pointer z-20 ${
                    isSelected ? "text-[#ffaae6] scale-105" : "text-white/60 hover:text-white"
                  }`}
        ...
        <div className="relative z-20 max-w-xl mx-auto px-6 py-12 text-center flex flex-col items-center lg:absolute lg:top-[33%] lg:left-[45%] lg:translate-x-0 lg:max-w-xl lg:text-left lg:items-start lg:py-0">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            ...
            <span style={{ color: "#21164c", fontWeight: 800, fontSize: 13 }}>Định nghĩa</span>
          </div>
          ...
          <p className="adora-body text-sm md:text-base leading-[1.6] mb-8 text-[#5f5f69] max-w-md">
            Bản chất hình học và các định nghĩa toán học cốt lõi của tích phân xác định.
          </p>
          <div className="flex gap-4">
            <button
              onClick={() => document.getElementById("definitions-grid")?.scrollIntoView({ behavior: "smooth" })}
      ...
      {/* ====== CARDS GRID ====== */}
      <div id="definitions-grid" style={{ background: "#f8f8f8", padding: "64px 5% 80px" }}>
        <div style={{
            ...
            >
              ...
              <div style={{ padding: "14px 18px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <span style={{
                  ...
                  border: `1px solid ${item.tagColor}`, color: item.tagColor,
